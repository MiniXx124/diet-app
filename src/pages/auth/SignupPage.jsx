...
function PasswordStrength({ password }) {
  ...
  const checks = [
    ...
    { label: '英字含む', ok: /[a-zA-Z]/.test(password) },
    { label: '数字含む', ok: /\d/.test(password) },
  ]
  const score = checks.filter(c => c.ok).length

  const barColor = score === 3
    ? 'bg-blue-500'
    : score === 2
    ? 'bg-yellow-400'
    : 'bg-red-400'
        ...
        {checks.map(c => (
          <span
            ...
            className={`text-xs flex items-center gap-1 ${
              c.ok ? 'text-blue-600' : 'text-gray-400'
            }`}
          >
            {c.ok ? '✓' : '○'} {c.label}
          </span>
...
export default function SignupPage() {
  ...
  const handleSignup = async (e) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        data: { full_name: username || null },
        emailRedirectTo: `${window.location.origin}/auth/callback`,
      },
    })

    if (error) {
      setError(ERROR_MESSAGES[error.message] ?? '登録に失敗しました。もう一度お試しください')
      setLoading(false)
    } else if (data.session) {
      // メール確認OFF: 即座にセッション取得 → App.jsx のルーティングが自動遷移
    } else {
      // メール確認ON: 確認メール送信済み画面を表示
      setDone(true)
    }
  }
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-5 py-10">
          <div className="w-full max-w-sm bg-white rounded-2xl border border-gray-200 p-8 shadow-sm text-center">
            ...
            <p className="text-gray-500 text-sm mb-6 leading-relaxed">
              <strong className="text-gray-700">{email}</strong> に確認メールを送りました。
              <br />メール内のリンクをクリックして登録を完了してください。
            </p>
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-700 text-left space-y-1">
              <p className="font-semibold">届かない場合</p>
              <p>・迷惑メールフォルダを確認する</p>
              <p>・数分待ってから再度試す</p>
            </div>
            <Link
              to="/login"
              className="mt-6 inline-block text-sm text-blue-600 font-semibold hover:underline"
            >
              ← ログイン画面に戻る
            </Link>
          </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-5 py-10">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-500 mt-1">まずはアカウントを作成しましょう</p>
          </div>
